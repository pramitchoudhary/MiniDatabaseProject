...
class IX_IndexHandle {
 public:
  ...
  // Variables
  PF_FileHandle *ix_pfHandleForIndexPage;
  string 		ix_indexName;
...
class IX_IndexScan {
 public:
  ...
  bool validateData (CompOp opObj, IX_KeyValue compareVal, IX_KeyValue retrievedData);

  vector<RID> m_scanRecord;
  vector<RID>::iterator m_itr;
  int m_count;
  AttrType m_type;
};
